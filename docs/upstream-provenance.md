...
- Constellation is published as `@celestial/ui` with a curated 47-builder barrel. Imported modal and contextual surfaces were updated to reflow on resize and retain visible close controls plus Escape dismissal; the cell-accurate status bar is a builder, while keyboard-help and context-menu composition remain public helpers outside the count.
