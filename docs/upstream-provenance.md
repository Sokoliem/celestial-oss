...
- Compass was rebuilt as a dependency-free headless subset: the donor's Nebula/Nova animated-router, preload, and render-to-string surfaces were excluded in favor of validated local URLs, deterministic matching, immutable history, and modal-safe screen navigation.
- Constellation is published as `@celestial/ui` with a curated 49-builder barrel. Imported modal and contextual surfaces were updated to reflow on resize and retain visible close controls plus Escape dismissal. The donor scrollbar and virtual-list prototypes were rebuilt around exact clipped rows, live geometry and item synchronization, stable keys, callback isolation, and the current element-mouse contract. The cell-accurate status bar is a builder, while keyboard-help and context-menu composition remain public helpers outside the count.
