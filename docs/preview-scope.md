...
- `examples/celestial-showcase` combines the entire focused preview into an adaptive, mouse-first Flight Deck with eight paged labs, a machine-checked 17-package/46-builder ledger, Rosetta locale and bidi receipts, deep Orbit and rich-rendering instruments, target-specific right-click menus, and Horizon windows, shelf, snap, tile, and session behavior.
