...
export const SHOWCASE_PACKAGE_COVERAGE = [
  ...
  { packageName: '@celestial/test', lab: 'Smoke', evidence: 'test', capabilities: ['headless app', 'screen queries', 'PTY harness'] },
  {
    ...
    evidence: 'live',
    capabilities: ['52 builders', 'context menu helper', 'responsive surfaces', 'headless app shell'],
  },
...
export const UI_BUILDER_COVERAGE = [
  ...
  { name: 'toolCall', evidence: 'interactive', capabilities: ['theme', 'pointer', 'keyboard', 'states', 'a11y', 'responsive'] },
  { name: 'diffViewer', evidence: 'rendered', capabilities: ['theme', 'states', 'a11y', 'responsive'] },
  { name: 'inlinePrompt', evidence: 'rendered', capabilities: ['theme', 'keyboard', 'states', 'a11y', 'responsive'] },
] as const satisfies readonly ShowcaseBuilderCoverage[];

/**
 * Public `@celestial/ui` runtime exports that are deliberately NOT curated
 * component builders: theme contracts, validators, layout measurers, hit-test
 * helpers, app-shell composition primitives, context-menu helpers, and the
 * inline-prompt app factories. The export surface is verified against
 * UI_BUILDER_NAMES ∪ UI_NON_BUILDER_EXPORTS by exact set equality in
 * app.test.ts, so a new public export fails the suite until it is consciously
 * classified as a builder or a helper.
 */
export const UI_NON_BUILDER_EXPORTS = [
