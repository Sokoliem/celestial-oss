...
export function boxPlot(opts: BoxPlotOpts): StatsChartResult {
  ...
  if (opts.data.length === 0) return makeResult(c);
  ...
  function mapY(val: number): number {
    return pxH - 1 - Math.round(((val - globalMin) / range) * (pxH - 1));
  }
...
export function candlestick(opts: CandlestickOpts): StatsChartResult {
  ...
  let globalMax = -Infinity;
  for (const d of opts.data) {
    globalMin = Math.min(globalMin, d.low);
    globalMax = Math.max(globalMax, d.high);
  }
  const range = globalMax - globalMin || 1;
...
export function histogram(opts: HistogramOpts): HistogramResult {
  ...
  if (opts.preBinned) {
    ...
    const numBins = opts.bins ?? Math.max(1, Math.ceil(Math.sqrt(data.length)));
...
export function bullet(opts: BulletOpts): StatsChartResult {
  ...
  const allNums = [opts.value, ...(opts.ranges ?? [])];
  if (opts.target !== undefined) allNums.push(opts.target);
  const dataMax = opts.max ?? Math.max(...allNums);
  ...
  function mapH(val: number): number {
    return Math.round(((val - dataMin) / range) * (pxW - 1));
  }
  function mapV(val: number): number {
    return pxH - 1 - Math.round(((val - dataMin) / range) * (pxH - 1));
  }
  ...
  if (opts.target !== undefined) {
    ...
    if (horiz) {
      const tx = mapH(opts.target);
      c.line(tx, 0, tx, pxH - 1);
    } else {
      const ty = mapV(opts.target);
...
export function waterfall(opts: WaterfallOpts): StatsChartResult {
  ...
  for (const d of opts.data) {
    if (d.isTotal) {
      ...
      cumulative.push(running + d.value);
      running += d.value;
    }
  }
