...
export function slider(config: SliderConfig): ComponentDescriptor<SliderModel, SliderMsg> {
  ...
  function valueAt(index: number): number {
    ...
    const safeIndex = boundedInteger(index, 0, 0, width - 1);
    const ratio = safeIndex / (width - 1);
    return clampToStep(interpolateRange(min, max, ratio), min, max, step);
  }

  function applyValue(model: SliderModel, value: number, dragging: boolean): SliderModel {
    const currentValue = clampToStep(model.value, min, max, step);
    const nextValue = clampToStep(value, min, max, step, currentValue);
    if (nextValue !== currentValue) config.onChange?.(nextValue);
  ...
  return {
    ...
    update(msg: SliderMsg, model: SliderModel): [SliderModel, Cmd<SliderMsg>] {
      switch (msg.type) {
        case 'increment': {
          const currentValue = clampToStep(model.value, min, max, step);
          const nv = clampToStep(currentValue + step, min, max, step, currentValue);
          if (nv !== currentValue) config.onChange?.(nv);
        ...
        case 'increment-large': {
          ...
          const nv = clampToStep(currentValue + largeStep, min, max, step, currentValue);
          if (nv !== currentValue) config.onChange?.(nv);
          return [{ ...model, value: nv }, Cmd.none()];
        }
    ...
    view(model: SliderModel): VNode {
      ...
      const value = clampToStep(model.value, min, max, step);
      const ratio = Math.max(0, Math.min(1, rangeRatio(value, min, max)));
      ...
      setVNodeMeta(node, {
        ...
        a11y: { role: 'slider', label: config.label, valueNow: value, valueMin: min, valueMax: max },
      });
      return node;
    },
